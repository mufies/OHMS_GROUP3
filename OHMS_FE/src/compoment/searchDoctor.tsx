...
export default function searchDoctor() {
    ...
    const handleSearchButtonClick = () => {
        if (searchInput.trim()) {
            ...
            // Tự động gửi tin nhắn sau khi modal mở
            setTimeout(async () => {
    ...
    const handleSubmitChat = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
        ...
        if (input && input.value.trim()) {
            ...
            // Tự động gọi getRecommendation nếu message chứa triệu chứng
            setTimeout(async () => {
            ...
            {showRecommendation && lastRecommendation && lastRecommendation.needMoreInfo && (
                <div className="flex justify-start mb-4">
                    <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 border-2 border-yellow-400 rounded-lg p-4 max-w-sm">
                        <h4 className="font-bold text-yellow-900 mb-3">📋 Thêm thông tin cần thiết</h4>
                        <p className="text-sm text-gray-700">{lastRecommendation.followUpQuestion || 'Vui lòng cung cấp thêm chi tiết để đưa ra chẩn đoán chính xác hơn.'}</p>
                    </div>
                </div>
            )}
